...
# Install the given packages via the distro package manager, prefixed with
# sudo when needed. Returns 1 when no supported package manager exists.
install_pkg() {
  local runner
  runner="$(pkg_runner)"
...
# curl and git are required. On Termux they are installed via pkg, together
# with unzip (opencode release) and ripgrep (opencode runtime dependency).
# pkg upgrade runs first because Termux requires a consistent package set:
# a mismatched one breaks the chromium install with "cannot locate symbol"
# link errors (e.g. ffmpeg vs libplacebo). An interrupted update can also
# corrupt libc++_shared.so, which `pkg upgrade` does not repair (packages are
# already "up to date"); reinstalling libc++ and fixing broken packages
# resolves that state.
#
# On non-Termux the prerequisites are installed via the distro package
# manager when missing, mirroring the Termux branch above. A fresh Arch WSL2
# ships neither git (not in the base group) nor which (dropped from base)
# nor unzip (required by the bun installer), and the official opencode
# installer calls `which opencode` (opencode.ai/install line 223), so with
# `set -e` a missing command aborts the whole install.
...
# bun — runtime used by the Playwright MCP command and dependency installs.
...
# proot's link2symlink converts hardlinks to .l2s symlinks, which breaks
# bunx and bun install. Only needed under proot; on a normal system the
# default hardlink backend is faster. Irrelevant on Termux, where hardlinks
# work natively.
configure_bun_options() {
  if is_termux || ! command -v proot >/dev/null 2>&1; then
    return
  fi
...
# opencode — the application itself. Upstream ships no Android binary and
  ...
  if is_termux; then
    ...
    if ! command -v opencode >/dev/null 2>&1; then
      if [ "$(uname -m)" != "aarch64" ]; then
        die "The Termux opencode build only supports aarch64; got $(uname -m)."
      fi
...
# ---------------------------------------------------------------------------
# Step 4b — PATH for future shells
# ---------------------------------------------------------------------------

# On a fresh distro (e.g. a new Arch WSL2) there is no ~/.bashrc, so the bun
# and opencode installers skip their PATH setup and the binaries are only
# reachable in the current shell. Write the exports ourselves, creating
# ~/.bashrc when missing; the grep checks make this idempotent across
# re-runs.
persist_paths() {
  local rc
  if is_termux; then
...
# Clone, update, or back up and replace the config directory.
setup_config_dir() {
  local remote backup_dir
  if [ -n "$SCRIPT_DIR" ] && [ "$SCRIPT_DIR" = "$CONFIG_DIR" ]; then
    ...
    if [ "$remote" = "$REPO_URL" ]; then
      ...
      # fast-forward pull fail; warn and keep the existing checkout instead
...
# package.json is gitignored, so recreate it when missing. allowScripts
# pre-approves the msgpackr-extract native build so npm 11.6+ (which blocks
# dependency install scripts by default) installs without warnings.
create_package_json() {
...
# Install dependencies (plugin SDK) with bun on every platform, including
# Termux. The allowScripts field in package.json is npm-specific and ignored
# by bun; it is harmless to keep. On Termux the bun cache is pre-warmed with
# the pinned MCP server so the first launch is fast.
  ...
  if is_termux; then
    if ! bun install; then
      ...
      if ! BUN_OPTIONS="--backend=copyfile" bun install; then
        die "bun install failed on Termux. Try: pkg reinstall bun, then re-run this script."
      fi
    fi
...
# ---------------------------------------------------------------------------
# Step 8 — Playwright browser
# ---------------------------------------------------------------------------

# Install through the MCP package itself so the chromium revision always
# matches the playwright-core bundled with @playwright/mcp; a bare
# `bunx playwright install` can drift to a different revision and break the
# MCP server.
...
# ---------------------------------------------------------------------------
# Step 9 — chromium system dependencies
# ---------------------------------------------------------------------------

# The MCP server launches a real chromium binary, which needs shared
# libraries that a minimal container does not ship. Playwright's own
# `install-deps` only covers Debian/Ubuntu and Alpine, so the lists are
# maintained here per distro. On Termux the chromium package pulls its own
# bionic dependencies, so the distro lists are skipped.
...
# ---------------------------------------------------------------------------
# Step 10 — verify chromium libraries
# ---------------------------------------------------------------------------

# Verify the chromium binary resolves every shared library it needs. On
# Termux the binary is the native chromium-browser from the x11-repo and
# ldd is not part of the default install, so this check is skipped. The
# paths found here are reused by the smoke test (step 11).
verify_chromium_libraries() {
  CHROME_BIN=""
...
# ---------------------------------------------------------------------------
# Step 11 — chromium smoke test
# ---------------------------------------------------------------------------

# Launch the exact chromium binary the MCP server uses. Headless mode uses
# the chromium-headless-shell build; prefer it and fall back to the full
# chromium build (which is what Termux uses).
smoke_test_chromium() {
  local smoke_bin="${CHROME_HEADLESS_SHELL:-$CHROME_BIN}"
...
# ---------------------------------------------------------------------------
# Step 12 — Termux MCP rewrite
# ---------------------------------------------------------------------------

# On Termux the repo's bun-based MCP command is rewritten to run under bun
# (bunx --bun) against the native chromium binary, pinned to
...
patch_mcp_for_termux() {
  ...
  sed -i \
    "s|^\(\s*\)\"command\": \[\".*@playwright/mcp.*|\1\"command\": [\"bunx\", \"--bun\", \"@playwright/mcp@$MCP_VERSION\", \"--headless\", \"--no-sandbox\", \"--executable-path\", \"$PREFIX/bin/chromium-browser\", \"--config\", \"$CONFIG_DIR/playwright-mcp.json\"],|" \
    opencode.jsonc
  ...
  sed -i '/"command": \["bunx"/a\'"$env_block" opencode.jsonc
}

# ---------------------------------------------------------------------------
# Step 12b — rewrite MCP command on Linux/macOS
# ---------------------------------------------------------------------------

# The committed config ships a Termux command (bunx --bun, --executable-path,
# Termux --config path) that must not survive here. The command is rewritten
# for the local platform and pinned to the same @playwright/mcp version used
# for the browser install in step 8, so the chromium revision the server
# launches always matches the one install.sh downloaded. Without the pin,
# `@latest` resolves at opencode startup to a newer version whose
# playwright-core expects a different browser revision. The Termux
# environment block (PLAYWRIGHT_BROWSERS_PATH and the PWTEST/PWMCP Android
# workarounds) is dropped too: on Linux those variables point at nonexistent
# /data/data/com.termux paths and break browser discovery.
rewrite_mcp_for_linux() {
  if is_termux; then
    return
  fi
  if grep -q '"@playwright/mcp@' opencode.jsonc; then
    ...
    sed -i \
      "s|^\(\s*\)\"command\": \[.*\"@playwright/mcp@[^\" ]*\".*|\1\"command\": [\"bun\", \"x\", \"@playwright/mcp@$MCP_VERSION_LINUX\", \"--headless\", \"--no-sandbox\", \"--config\", \"$CONFIG_DIR/playwright-mcp.json\"],|" \
      opencode.jsonc
    sed -i '/"environment": {/,/},/d' opencode.jsonc
  else
...
# ---------------------------------------------------------------------------
# Step 13 — fix absolute paths
# ---------------------------------------------------------------------------

# The committed config ships Termux paths (/data/data/com.termux/files/home
# and /data/data/com.termux/files/usr); rewrite them for this machine. The
